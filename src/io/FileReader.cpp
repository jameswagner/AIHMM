...
void FileReader::readFiles(std::vector<std::unique_ptr<Individual>>& individuals, int startChromosome, int endChromosome, const std::string& fileNamePrefix) {
    
    for (unsigned int individualIter = 0; individualIter < individuals.size(); individualIter++) {
        individuals[individualIter] = std::make_unique<Individual>();
    }

    char line[100000];
    for (unsigned int individualIter = 0; individualIter < individuals.size(); individualIter++) {
        for (int chromosomeIndex = startChromosome; chromosomeIndex <= endChromosome; chromosomeIndex++) {
            auto chromosome = std::make_unique<Chromosome>();
            individuals[individualIter]->addChromosome(std::move(chromosome));
        }
    }
    ...
    for(int chromosomeIter = 1; chromosomeIter <=endChromosome; chromosomeIter++)  
      std::cout << "This is the size of chromosome" << chromosomeIter << " " << individuals[0]->getChromosomes()[chromosomeIter-1]->getFullExpressions(1).size() << std::endl;
